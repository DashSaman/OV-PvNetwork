...
if [[ -f /opt/ov-node/core/routers/router.py ]] \
   && grep -q '_ov_dashboard_network_snapshot' \
        /opt/ov-node/core/routers/router.py
then
    sed -i \
        '/^[[:space:]]*status.update(_ov_dashboard_network_snapshot())/d' \
        /opt/ov-node/core/routers/router.py

    sed -i \
        '/^[[:space:]]*status.update(_ov_openvpn_online_snapshot())/d' \
        /opt/ov-node/core/routers/router.py
