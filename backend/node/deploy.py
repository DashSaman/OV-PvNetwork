...
DEPLOY_TIMEOUT = int(os.getenv("OV_NODE_DEPLOY_TIMEOUT", "900"))
JOB_TTL = int(os.getenv("OV_NODE_DEPLOY_JOB_TTL", "86400"))
...
def _domain_payload(path: str) -> str:
    asset_root = os.getenv("OV_DOMAIN_ASSET_ROOT")
    source = (
...
rollback() {{
  ...
  if [[ $rc -ne 0 && $changed -eq 1 ]]; then
    ...
    echo "OV_ROLLBACK|Previous OV Node restored"
  fi
  rm -rf "$DOMAIN_BACKUP"
  exit $rc
}}
...
# OV_PVNETWORK_RUNTIME_COMPAT_V1
...
echo "OV_WARN|Domain history is optional and has been deferred; core node installation continues"
domain_changed=0
