...
class RenameProductionContractTests(unittest.TestCase):
    def test_release_version_is_current(self):
        self.assertEqual((ROOT/'VERSION').read_text().strip(),'1.0.28')
        self.assertEqual(json.loads((ROOT/'frontend/package.json').read_text())['version'],'1.0.28')
        lock=json.loads((ROOT/'frontend/package-lock.json').read_text())
        self.assertEqual(lock['version'],'1.0.28'); self.assertEqual(lock['packages']['']['version'],'1.0.28')
