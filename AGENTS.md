...
- `v1.0.9` — RELEASED — `PVN-029` opt-in Router/OpenVPN compatibility on an isolated listener/profile; normal certificate-only OpenVPN remains the default and is not modified.
  - Release scope: per-Node opt-in Router/legacy listener on a separate port/subnet; certificate + password dual authentication; per-user/per-node verifier-only credentials; dedicated Router profile generation; capability upgrade detection; Router health/presence visibility with cross-listener Common Name deduplication.
  - Verification: exact-head branch CI run `35458976053` PASS and PR #32 CI run `35459640868` PASS; final behavior merged as main commit `6aa55f20b33a3a8ebc2633254e1ab19ea2248c1f`, with main CI run `35459813811` PASS including 123 unit/governance tests, Router safety contracts, real dual-auth OpenVPN handshake, frontend build/audit/bundle budget and full browser smoke matrix.
  - Production rollback point: `/root/pvnetwork-deploy-backups/v1.0.9-pvn029-20260919-180252`; application archive and native PostgreSQL dump passed tar/`pg_restore -l` verification before mutation; Nginx and firewall snapshots were also captured.
  - Production deployment: additive Alembic migration `d3e4f5a6b7c8` applied successfully; exact main tree deployed with 399/399 tracked-file parity and zero frontend-dist differences. Public traffic was cut to the validated 19002 canary while canonical 19001 was updated, then returned to 19001. Only `pvnetwork-panel.service` restarted; normal OpenVPN PID `1022714` and `server.conf` hash remained unchanged.
...
- `v1.0.12` — RELEASED — `PVN-032` guarded runtime main-admin/panel-path settings.
  ...
  - Production rollback point: `/root/pvnetwork-deploy-backups/v1.0.12-pvn032-20260920-213040`; source/config snapshots, firewall/Nginx evidence and native PostgreSQL dump were checksum/list verified before final rollout validation.
...
- `v1.0.13` — RELEASED — `PVN-894` fail-closed Production canary retirement.
  ...
  - Release completion: final main `ca6935881bae6262b3944c1fad5b3fc1897f3859`; GitHub Release `392603827`; public re-download verification PASS with SHA256 `56dd6ac517d2ae63009615e3571b24ae0442ab5b01d22e8d5ddc5a837f8d7106`, VERSION `1.0.13`, 528 entries and zero forbidden private-file extensions.

- `v1.0.14` — RELEASED — `PVN-585` security regression.
  - Reproduced/fixed: canonical loopback-proxy client-IP parsing for allowlist self-lockout; API-token rejection from interactive main-admin security operations; API-token prefix allowlist (`pvn_` / `ovp_` only); exact API-prefix scope classification replacing substring matching.
  - Verified-safe controls: JWT tamper/expiry/main-admin generation, disabled delegated-admin stale sessions, identifier/ownership boundaries, panel status-token replay, SQL parameterization guard, protected-route inventory, secret redaction, exact-origin CORS, bearer-only CSRF posture, React XSS escaping and sensitive-file modes.
  - Permanent gates: focused Python security suite, mocked Chromium XSS smoke, credential-free GET-only Production security probe, existing dependency/Bandit/OpenVPN/browser/secret gates preserved. PR #43 exact-head CI run `35545727525` PASS; merged runtime main `d3a4e9d6f7c0714e1dcfc33c880f336fb538a846` main CI run `35546049186` PASS.
  - Production rollback/canary: verified backup `20260920-235940` passed checksum and PostgreSQL restore-list validation. Exact merged-main canary on 19002 reported v1.0.14 and public UI/API 200; canonical 19001 was upgraded, `pvnetwork-canary-retire-guard` returned `CANARY_RETIRE_SAFE=YES`, then canary retirement left public health/panel/users at 200 with zero observed 502 recurrence.
  - Production security evidence: credential-free read-only probe PASS (health 200, docs/OpenAPI 404, protected routes 401, headers/CORS/file-mode checks); full panel smoke PASS with 98 effective routes, Alembic/DB/timers PASS; synthetic loopback login throttle reached 429 at configured limit 10 with `Retry-After: 60`. Node service identity stayed unchanged and both normal/Router OpenVPN config hashes stayed unchanged.
  - Operational observation: during the rollout window, the pre-existing periodic usage/limit lifecycle independently disabled an eligible user and the Node's existing status-change handler restarted the normal OpenVPN service. Timeline and code-path evidence attribute this to `enforce_user_limits -> /sync/user -> restart_openvpn_service`, not to the v1.0.14 deployment; no OpenVPN config mutation occurred and no later restart was observed. Removing whole-service restarts from user lifecycle is deferred to the existing service-preservation/network-regression stream (`PVN-376` / `PVN-398`) rather than expanding PVN-585.
  - Release completion: final main `d75a871f3d792e58c9266fb4f574431d25865a16`; final main CI `35547142749` PASS; GitHub Release `392623305`; public re-download SHA256 `1f67a1b11bb33391f86ec1bf3da574af40ea99812d3005955c6098de8f4802b9` PASS; VERSION `1.0.14`, 540 entries, forbidden private files 0.

- `v1.0.15` — [x] RELEASE CANDIDATE VERIFIED — `PVN-376` / `PVN-398` per-user OpenVPN lifecycle service preservation.
  - Reproduced Production defect: legacy Node `change_user_status()` restarted `openvpn-server@server` for a single user, and the live Delete path depended on a missing interactive installer.
  - Implemented contract: Enable/Disable/Delete contain no whole-service OpenVPN restart; Disable/Delete disconnect only the exact target CN through the management socket; Delete uses EasyRSA directly, regenerates/publishes CRL atomically, cleans only target leaf artifacts, and retries safely after a prior revoke/CRL-publish interruption.
...
- `v1.0.31` — RELEASED — `PVN-1015` Vengeance-style skin + speed.
  ...
  - Speed (owner-authorized, host level): nginx gzip_types enabled (JS/JSON/SVG/webmanifest) and a hashed-asset location with `Cache-Control: public, max-age=31536000, immutable`; main bundle measured 981827 → 297078 bytes gzipped; config backed up at `/root/pvnetwork-deploy-backups/nginx-20260925-135947`, `nginx -t` + graceful reload; BBR+fq confirmed already active and persisted in sysctl.
  - CPU note: the reported pinning was the v1.0.28→v1.0.30 Telegram repeat loop (monitor + telegram sends); post-hotfix top shows panel ≈2%, paqet/x-ui unchanged.
  - Release completion: CI PASS on `8dfe2bd`; tag `v1.0.31`; GitHub Release `396662894` with artifact + SHA256 (public re-download verified); Production deployed (backend metadata + frontend dist; served CSS confirmed to contain the skin tokens; live login page screenshot verified). A partial asset cleanup during verification was caught and fully restored from the same verified tar before final checks; `/healthz` 200 `1.0.31`, 0 errors, unrelated services untouched.
...
- `v1.0.29` — RELEASED — `PVN-1013` installable panel (PWA) + mobile users-table cards.
  ...
  - Post-deploy: public `/hajsaman/assets/manifest.webmanifest` and icon 200; index links the manifest; 0 errors; unrelated services untouched.
...
- `v1.0.26` — RELEASED — `PVN-1012` Router password viewable in the admin panel.
  ...
  - Release completion: three CI iterations — the first correctly caught the model column missing from the Router class and legacy hash-only contracts in three tests, which were updated to the ciphertext contract (encrypted-not-plaintext, decrypt round-trip, admin reveal); final exact-head CI PASS on `47fa85d`; immutable tag `v1.0.26`; GitHub Release `395959229` with artifact + SHA256; public re-download verification PASS (`5f2cbce68672981ff927219f2fe05bebb5ba98695de31f8e8849cb758fde31de`).
  - Production deployment (owner-authorized): fresh PostgreSQL backup (62 tables) at `/root/pvnetwork-deploy-backups/v1.0.26-pvn1012-*`; Alembic upgraded `e7f8a9b0c1d2 → f8a9b0c1d2e3` and `password_ciphertext` confirmed in the live table; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.26`; live bundle contains the stored-password UI; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched.
...
- `v1.0.24` — RELEASED — `PVN-1009` subscription runtime revived + MikroTik on-create panel fixed.
  ...
  - Fix 2: response is {name, uuid} (single consumer; legacy fallback by name lookup); results panel shows node server address, per-node Router profile download and a built-in MikroTik/RouterOS tutorial in 13 languages (plain-text render — the raw-HTML-sink security gate rejected the initial dangerouslySetInnerHTML implementation and it was rewritten as pre-line text).
  - Release completion: two CI iterations caught by the gates (stale vendored-path assertion in the v1.0.23 QR test; the raw-HTML-sink security test) were fixed; final exact-head CI PASS on `53f522b`; immutable tag `v1.0.24` pushed; GitHub Release `395909055` published with artifact + SHA256; public re-download verification PASS (`29d34494bc07d92150eae0743593138ec78a33671b4310bf6b197a4c1d77aca6`).
  - Production deployment (owner-authorized): 24-file backend/template delta + admin frontend dist; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.24`; QR asset publicly served by the backend with `Content-Type: text/javascript` (nginx untouched); 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched.
  - Live browser verification (the same reproduction that found the defect): the renewal countdown now computes (shows the correct per-user state instead of the eternal placeholder), `window.qrcode` and `PNQR` execute, and clicking the page-QR button renders the QR SVG overlay with the correct subscription URL caption.
...
- `v1.0.23` — RELEASED — `PVN-211` smart subscription page (QR + per-device guides).
  ...
  - Release completion: two intermediate CI failures were caught and fixed by the gates themselves (quote-style assertion in the new test; the 44px touch floor for the new QR buttons; vendored library excluded from ESLint); final exact-head CI PASS on `fb65666`; immutable tag `v1.0.23` pushed; GitHub Release `395885453` published with artifact + SHA256; public re-download verification PASS (`b54dc816a8efbd2cbca7ffd5343f7925616a53a27d5b953e2299b06791e256e8`).
  - Production deployment (owner-authorized): 20-file template/metadata delta plus the vendored `sub_clients/qr/qrcode.js` installed with 0644; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.23`; QR asset publicly served (HTTP 200 through nginx, no nginx reload needed); a real user subscription page renders HTTP 200 with the QR overlay, page-QR button, device-grid and `PNQR` runtime present; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched.
...
- `v1.0.22` — RELEASED — `PVN-1008` reliable automatic node deployment.
  ...
  - Release completion: first CI run on `14b5720` correctly failed two of the new test assertions (missing default, two-line comment substring) and was fixed on `93f6b94` (CI PASS); immutable tag `v1.0.22` pushed; GitHub Release `395660747` published with artifact + SHA256; public re-download verification PASS (`cbe0f44136ded9b83e186792847fccff5bdc35de6d385db0258e36daa40379b4`).
  - Production deployment (owner-authorized): 21-file backend/metadata delta plus frontend dist through the additive-assets/atomic-index procedure; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.22`; `py_compile` PASS with `PVNETWORK_PANEL_SOURCE_AUTODETECT_V1` present; public `/hajsaman/` 200 serving `index-D9RrGbob.js` containing the new Add-Node keys; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched; stale chunks removed.
...
- `v1.0.21` — RELEASED — `PVN-1006` full language uniformity + `PVN-1007` Router credentials at user creation (owner-directed combined patch; the owner requested both changes together in a single instruction).
  ...
  - PVN-1006 permanent gates: zero Arabic-script values in non-Persian catalogs (native language-name keys exempt), zero Persian-only characters in the Arabic catalog, and zero raw Persian text outside translation calls in any component.
  - PVN-1007 contract: the Add User dialog gains a Router / MikroTik checkbox; after successful creation the client calls `POST /router-openvpn/users/{uuid}/nodes/{nodeId}/credential` for each selected node, then displays a one-time results panel with per-node username/password (or per-node failure detail) before closing. The normal certificate-only OpenVPN profile and the existing post-creation Router modal are unchanged; focused source-contract test `tests/test_create_user_router_credentials.py`.
  - Bundle: fully translated catalogs grow the main chunk to 942.4 kB; CI budget raised to 950000 bytes (`PVN-1004` lazy language loading remains the registered shrink path).
  - Release completion: exact-head CI PASS on `5443854`; immutable tag `v1.0.21` pushed; GitHub Release `395640629` published with artifact + SHA256; public re-download verification PASS (`e63512d2ff360510809ee69c80a9ce2b6374bfcee06dc63d9d184e1f0e7b07e6`).
  - Production deployment (owner-authorized): 20-file backend/metadata delta plus full frontend dist through the additive-assets/atomic-index procedure; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.21`; public `/hajsaman/` 200 serving `index-BHSCpt3Y.js` with the new `backup.*`/`createUserRouter*` keys present in the bundle; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched; stale hashed chunks removed after the switch.
...
- `v1.0.20` — RELEASED — `PVN-1005` uniform language switching.
  ...
  - Release completion: exact-head CI PASS on `4323112` (bundle budget 860000, largest chunk 850.6 kB); immutable tag `v1.0.20` pushed; GitHub Release `395610335` published with artifact + SHA256; public re-download verification PASS (`b0b9d43c9ff737753a57ac7f646e76669ea9768a729e5c7f79f79db1d51272aa`; a first asset upload landed in a partial state and was deleted/re-uploaded before verification).
  - Production deployment (owner-authorized): 19-file backend/metadata delta plus full frontend dist through the additive-assets/atomic-index procedure; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.20`; public `/hajsaman/` 200 serving `index-DssRqTk6.js`; SPA index still `cache-control: no-cache`; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched.
...
- `v1.0.19` — RELEASED — `PVN-1003` white-screen regression fix, index no-cache, i18n completion and Production database rebrand.
  ...
  - Completion: 35 missing UI keys (Quick Edit, node selector, Router/MikroTik, rename) translated in all 11 secondary languages — every shipped language now covers the full 407-key catalog.
  - Production database rebrand: legacy database/login role migrated to `pvnetwork_panel` backup-first (fresh verified dump 62 tables; parity 31/31 tables, 71/71 users, 4/4 nodes, Alembic head `e7f8a9b0c1d2` preserved); table/schema/database ownership transferred to the application role; `.env` updated atomically; legacy database retained untouched as rollback at `/root/pvnetwork-deploy-backups/v1.0.19-pvn1003-20260924-110015` (also holds `env-pre-rename` and the pre-rename dump).
  - Known impact window: ~90 seconds of dashboard-live HTTP 500s between the `.env` switch and the ownership grant during the database rename; closed immediately and verified 0 errors/0 5xx afterwards. Owner notification: rotate the panel database password if desired — it was machine-generated during the migration.
  - Release completion: exact-head CI PASS on `61b25df0d485ba3a70cbda67f92d11aa01a9d21e2` (a first CI run on `372edf0` correctly failed the bundle-budget gate at 775000 before the documented raise to 820000); immutable tag `v1.0.19` pushed; GitHub Release published with `pvnetwork-panel-v1.0.19.tar.gz` + `.sha256`; public re-download SHA256 verification PASS (`ac4fa246aa6fc97ba629c3a49a4df5838eb4246030a7427d522d935af8e9a992`).
  - Production deployment: 20-file backend delta (hash-verified) + full frontend dist with the corrected `URLPATH` build applied through the additive-assets/atomic-index procedure; one `pvnetwork-panel.service` restart. Post-deploy: `/healthz` 200 `1.0.19`; public `/hajsaman/` 200 serving `index-D8qO8uRa.js`; SPA index carries `cache-control: no-cache` + ETag locally and publicly; unauthenticated API still 401; 0 Traceback/CRITICAL; `paqet`/`x-ui`/nginx untouched; new database serves 71 users.
...
- `v1.0.17` — RELEASED — `PVN-1000` debounced CPU threshold alerts.
  ...
  - Governance correction riding with this release: registers `PVN-1000` and the `PVN-1000..1099` post-registry hotfix range (the permanent 001–999 registry is fully allocated), repoints README/installer pins to the current release, and closes stale v1.0.0-era planning issues (#4–#12).
  - Verification evidence: exact-head CI PASS on `0fc6e28bd5f6e82316837ad8c2058a43d866b3f5` (v1.0.16 tag head `ac8db0cead35241d653d6b58eb995f9b92ca84ca` also CI-verified); immutable tags `v1.0.16`/`v1.0.17` pushed; both GitHub Releases published with `pvnetwork-panel-v*.tar.gz` + `.sha256` assets; public re-download SHA256 verification PASS (`4d215d1c09672ebf1da02479e9aa633a31201d30f6f4d08486c92d546a53c956` for v1.0.17).
  - Production rollback point: `/root/pvnetwork-deploy-backups/v1.0.17-pvn1000-20260924-101225` — application archive (gzip-verified, 9555 paths, includes runtime `.env`/`data/`) plus native PostgreSQL dump (58.8 MB, 62 tables listed by `pg_restore -l`); restore = untar over `/opt/pvnetwork-panel` + `pg_restore`.
  - Production deployment (owner-authorized, single narrow rollout of `v1.0.17` covering both v1.0.16 and v1.0.17): preflight confirmed live tree hash-matched `ac8db0c` and Alembic already at head `e7f8a9b0c1d2`; 20-file delta staged and md5-verified against git blobs before apply; `py_compile` PASS; only `pvnetwork-panel.service` restarted (active/running, MainPID 99352).
  - Post-deploy evidence: local `/healthz` 200 `{"status":"ok","version":"1.0.17"}`; panel `/hajsaman/` 200; public `https://open1.softarg.ir/healthz` 200 and `/hajsaman/` 200; unauthenticated `/api/users/` 401; 0 Traceback/CRITICAL/ERROR in panel journal since restart; official `pvnetwork-panel-smoke-test` **PASS** (all stages incl. RENAME_WORKER/TIMERS); monitor one-shot PASS with `threshold_counters` persisted for all 4 nodes and no spurious alert transitions; DB counts unchanged (71 users / 4 nodes / sessions intact); no migration required.
  - Unrelated services untouched: `paqet` PID 827, `x-ui` PID 872, `xray` PID 3910173 (started 06:29, pre-deployment), nginx active without restart; frontend dist intentionally not replaced (v1.0.17 changes no frontend source).
  - Known pre-existing condition (not changed by this release, recorded for a future PVN): `pvnetwork-panel-firewall-hardening.service` fails at boot with `required service is not active: ssh.service` (this host's SSH unit name differs) and correctly rolls back; host firewall state is left as-is.
