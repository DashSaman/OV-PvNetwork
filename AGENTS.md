...
- `v1.0.14` — RELEASE CANDIDATE — `PVN-585` security regression.
  ...
  - Permanent gates: focused Python security suite, mocked Chromium XSS smoke, credential-free GET-only Production security probe, existing dependency/Bandit/OpenVPN/browser/secret gates preserved.
  - Safety boundary: state-changing adversarial tests remain isolated; Production receives read-only probes only. Node and normal OpenVPN remain rollout isolation invariants. Production rollout, final main CI, immutable tag/assets and public SHA re-download verification remain release gates.
