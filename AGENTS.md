...
- `v1.0.7` — IN PROGRESS — `PVN-028` Production security hardening from the authorized 2026-09-19 review.
  - Scope: remediate vulnerable Python dependency set with compatibility tests and CI `pip-audit`; replace SSH first-seen auto-trust with host-key verification/pinning; migrate main-admin password verification away from plaintext comparison; restrict Production OpenAPI/Redoc; add tested security headers and tighter login throttle; prepare an explicit host-firewall allow policy only after complete listener/data-plane inventory.
  - Safety boundary: firewall, VPN/tunnel/Xray listeners and remote Node access are not mutated until their required ports/peers are proven and a rollback path is ready. Main-admin credential migration must preserve an emergency recovery path.
  - Private detailed audit evidence stays root-only on Production at `/root/pvnetwork-security-audit-20260919.md`; public repository notes remain sanitized and do not publish exploitable environment details.
  - Next: isolated v1.0.7 worktree → TDD security regression tests → dependency/auth/SSH/web hardening → full CI/security scans → canary/Production verification → release.
