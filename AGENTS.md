...
- `v1.0.15` — [~] IMPLEMENTATION / CI GATE — `PVN-376` / `PVN-398` per-user OpenVPN lifecycle service preservation.
  - Reproduced Production defect: legacy Node `change_user_status()` restarts `openvpn-server@server` for a single user, and the live Delete path depends on a missing interactive installer.
  - TDD contract: Enable/Disable/Delete must contain no whole-service OpenVPN restart; Disable/Delete disconnect only the target CN through the management socket; Delete revokes only the target cert, regenerates/publishes CRL atomically, and Node upgrade must install the lifecycle patch idempotently.
  - Production remains untouched until exact-head branch and merged-main CI are green; rollout requires verified backup plus OpenVPN PID/config-hash preservation evidence.
