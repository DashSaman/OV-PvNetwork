...
- PVN-1019 [x] Presence rate-dict hotfix (500s on /users/presence since split rates). Released in v1.0.35.
- PVN-1018 [x] Stable Speed (↓/↑) table column, chart bytes→bits unit fix, daily renewal digest. Released in v1.0.34.
- PVN-1017 [x] Split per-user ↓/↑ live speeds (node rx/tx patch + presence down/up/total) and per-node dashboard sparklines; renewal popup weekly. Released in v1.0.33.
- PVN-1016 [x] Live per-user speed badge in the Users table (panel-side usage-delta rates via /users/presence) and a recharts dashboard chart in a lazy chunk. Released in v1.0.32.
- PVN-1015 [x] Vengeance-style professional skin (token-level, single-file rollback) plus nginx gzip and immutable asset caching. Released in v1.0.31.
- PVN-1014 [x] Renewal-alert repeat hotfix: renewal keys excluded from the generic transition builder (silent-clear semantics only). Released in v1.0.30.
...
- PVN-1009 [x] Subscription runtime revived + MikroTik on-create panel fixed: path-scoped inline-script CSP for /sub, backend-served QR library with correct MIME, create-user returns {name, uuid}, Router results panel shows server address + profile download + RouterOS tutorial. Released in v1.0.24.
- PVN-1011 [x] Router readiness probe in Add User (live per-node capability check on checkbox tick, inline enable instructions, actionable credential-failure translations). Released in v1.0.25.
- PVN-1012 [x] Router/MikroTik password viewable in the admin panel: Fernet password_ciphertext storage, status-endpoint reveal, modal display with legacy-rotate hint, softened on-create warning. Released in v1.0.26.
- PVN-211 [x] Smart subscription page: page-level and per-server QR codes (vendored MIT qrcode-generator, same-origin, CSP-safe) plus per-device quick-connect guides in fa/en. Released in v1.0.23 (first item of the owner-approved track in ROADMAP.md).: optional auto-detected panel source IP (derived on the node from the SSH session instead of the broken hostname prefill), position-1 firewall rule insertion that survives empty INPUT chains, IPv6 allowlist branch, actionable post-install verification errors, and translated Add-Node SSH field labels. Released in v1.0.22.
