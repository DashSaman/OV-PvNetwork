...
- **v1.0.34 released:** `PVN-1018` stable speed column + 8× chart unit fix + daily renewal digest.
...
- **v1.0.25 released:** `PVN-1011` Router readiness probe — the Add User Router checkbox live-checks selected nodes and teaches the enable path; credential failures become actionable messages.
- **v1.0.24 released:** `PVN-1009` subscription runtime revived (path-scoped inline-script CSP; the v1.0.8 strict CSP had silently disabled the whole /sub page) and the MikroTik on-create panel fixed (uuid in create response, server address, profile download, RouterOS tutorial).
