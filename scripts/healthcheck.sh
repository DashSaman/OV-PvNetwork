...
if ! check_three "http://127.0.0.1:${PANEL_PORT}/openapi.json"; then
...
' /opt/ov-node/.env 2>/dev/null | tail -1)"
NODE_PORT="${NODE_PORT:-9090}"

if ! check_three "http://127.0.0.1:${NODE_PORT}/openapi.json"; then
  logger -t pvnetwork-panel-healthcheck "external node API failed three checks on port ${NODE_PORT}; restarting ov-node.service"
